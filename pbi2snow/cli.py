...
def print_summary(manifest):
    ...
    problem_tables = [t for t in manifest['tables'] if not t['success'] or t['confidence'] < 50]
    if problem_tables:
...
@click.command()
@click.option('--bim-file', 
              default='Model.bim', 
              help='Path to the Power BI model file (BIM/TMSL format)')
@click.option('--output-dir', 
              default='out_unified', 
              help='Output directory for SQL views')
...
@click.option('--enable-unpivot/--disable-unpivot',
              default=True,
              help='Enable UNPIVOT translation')
@click.option('--enable-union-all/--disable-union-all',
              default=True,
              help='Enable Table.Combine to UNION ALL translation')
@click.option('--enable-nested-joins/--disable-nested-joins',
              default=True,
              help='Enable Table.NestedJoin translation')
@click.option('--enable-text-functions/--disable-text-functions',
              default=True,
              help='Enable Text function translations')
@click.option('--enable-lookupvalue/--disable-lookupvalue',
              default=True,
              help='Enable LOOKUPVALUE translation')
@click.option('--enable-calculated-patterns/--disable-calculated-patterns',
              default=True,
              help='Enable calculated table pattern detection')
@click.option('--parse-m-expressions/--skip-m-parsing',
              default=True,
              help='Parse M expressions with Node.js parser')
@click.option('--optimize-where/--no-optimize-where',
              default=True,
              help='Optimize WHERE clause AND/OR logic')
@click.option('--clean-brackets/--keep-brackets',
              default=True,
...
@click.option('--dry-run',
              is_flag=True,
              help='Run without writing files (preview only)')
@click.option('--filter-tables',
              help='Comma-separated list of table names to process (process all if not specified)')
@click.option('--exclude-tables',
              help='Comma-separated list of table names to exclude')
@click.option('--confidence-threshold',
              type=int,
              default=0,
              help='Only output views with confidence >= threshold (0-100)')
def main(bim_file, output_dir, target_schema, mode, enable_unpivot, enable_union_all,
         enable_nested_joins, enable_text_functions, enable_lookupvalue,
         enable_calculated_patterns, parse_m_expressions, optimize_where,
         clean_brackets, verbose, dry_run, filter_tables, exclude_tables,
    ...
    try:
        ...
        with click.progressbar(tables_to_process, label='Translating') as tables:
            for table in tables:
                ...
                if verbose and not result['success']:
                    logger.warning(f"Failed to translate {table['name']}: {result.get('errors', [])}")
        ...
        if not dry_run:
            ...
            with open(combined_file, 'w') as f:
                ...
                f.write(f"-- Total views: {len([r for r in results if r['success']])}\n\n")
