...
class MToSQLTranslator:
    ...
    def translate(self, m_expr: str, table_name: str) -> TranslationResult:
        ...
        try:
            ...
            if not source_info:
                result.errors.append(f"Could not identify source table for {table_name}")
                result.sql = f"-- Could not identify source table\nSELECT * FROM {table_name}"
                return result
    ...
    def _extract_steps(self, m_expr: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if '=' in line_stripped and paren_depth == 0 and bracket_depth == 0 and brace_depth == 0:
                if not line_stripped.startswith('let') and not line_stripped.startswith('in'):
                    # Save previous step
                    if current_step:
    ...
    def _translate_operations(self, steps: List[Dict[str, Any]], 
                            source_info: Dict[str, Any],
                            result: TranslationResult) -> Dict[str, Any]:
    ...
    def _translate_text_functions(self, expr: str) -> str:
        ...
        expr = re.sub(r'Text\.From\s*\(\s*\[([^\]]+)\]\s*\)', r'CAST(\1 AS VARCHAR)', expr)
        
        # Text.Proper([column]) -> INITCAP(column)
        expr = re.sub(r'Text\.Proper\s*\(\s*\[([^\]]+)\]\s*\)', r'INITCAP(\1)', expr)
        
        # Text.BeforeDelimiter([column], delimiter)
        expr = re.sub(r'Text\.BeforeDelimiter\s*\(\s*\[([^\]]+)\]\s*,\s*"([^"]+)"\s*\)', 
                     r"SPLIT_PART(\1, '\2', 1)", expr)
        
        # Text.AfterDelimiter([column], delimiter)
        expr = re.sub(r'Text\.AfterDelimiter\s*\(\s*\[([^\]]+)\]\s*,\s*"([^"]+)"\s*\)', 
                     r"SPLIT_PART(\1, '\2', 2)", expr)
        
        # Text.Contains([column], "text")
        expr = re.sub(r'Text\.Contains\s*\(\s*\[([^\]]+)\]\s*,\s*"([^"]+)"\s*\)', 
                     r"\1 LIKE '%\2%'", expr)
        
        # Text.StartsWith([column], "text")
        expr = re.sub(r'Text\.StartsWith\s*\(\s*\[([^\]]+)\]\s*,\s*"([^"]+)"\s*\)', 
                     r"\1 LIKE '\2%'", expr)
        
        # Text.EndsWith([column], "text")
        expr = re.sub(r'Text\.EndsWith\s*\(\s*\[([^\]]+)\]\s*,\s*"([^"]+)"\s*\)', 
                     r"\1 LIKE '%\2'", expr)
        ...
        expr = re.sub(r'Text\.Replace\s*\(\s*\[([^\]]+)\]\s*,\s*"([^"]+)"\s*,\s*"([^"]+)"\s*\)', 
                     r"REPLACE(\1, '\2', '\3')", expr)
    ...
    def _translate_combine(self, expr: str) -> Optional[Dict[str, Any]]:
        ...
        if match:
            ...
            return {
                'type': 'combine',
                'tables': tables,
                'sql_parts': union_parts
            }
        
        return None
    
    def _translate_nested_join(self, expr: str, next_steps: List[Dict]) -> Optional[Dict[str, Any]]:
        ...
        if match:
            ...
            join_kind = match.group(5).strip() if match.group(5) else 'JoinKind.LeftOuter'
            ...
            for next_step in next_steps:
                if 'Table.ExpandTableColumn' in next_step['expression'] and join_name in next_step['expression']:
                    expand_match = re.search(
                        r'Table\.ExpandTableColumn\s*\([^,]+,\s*"' + join_name + r'"\s*,\s*\{([^}]+)\}',
                        next_step['expression']
                    )
                    if expand_match:
                        cols_str = expand_match.group(1)
                        expanded_cols = [col.strip().strip('"') for col in cols_str.split(',')]
                    break
    ...
    def _build_sql(self, sql_parts: Dict[str, Any]) -> str:
        ...
        if sql_parts.get('unpivot'):
            ...
            result_sql = f"SELECT * FROM ({result_sql}) AS pivoted\n{unpivot_info['sql']}"
        
        # Apply PIVOT if present
        if sql_parts.get('pivot'):
            pivot_info = sql_parts['pivot']
            result_sql = f"SELECT * FROM ({result_sql}) AS unpivoted\n{pivot_info['sql']}"
    ...
    def _extract_added_column(self, expr: str) -> Optional[Dict[str, str]]:
        ...
        match = re.search(r'Table\.AddColumn\s*\([^,]+,\s*"([^"]+)",\s*each\s+(.+?)\)', expr)
        if match:
    ...
    def _translate_column_expression(self, expr: str) -> str:
        ...
        # Handle if/then/else
        if ' if ' in expr.lower():
            match = re.search(r'if\s+(.+?)\s+then\s+(.+?)\s+else\s+(.+)', expr, re.IGNORECASE)
            if match:
    ...
    def _extract_duplicate_column(self, expr: str) -> Optional[Dict[str, str]]:
        ...
        match = re.search(r'Table\.DuplicateColumn\s*\([^,]+,\s*"([^"]+)",\s*"([^"]+)"\)', expr)
        if match:
    ...
    def _is_source_step(self, expr: str) -> bool:
        ...
        source_indicators = ['Source', 'Database', 'Schema', 'Snowflake.Databases', 'Sql.Database']
    ...
    def _extract_native_query(self, steps: List[Dict[str, Any]], table_name: str) -> str:
    ...
    def _check_unsupported_operation(self, expr: str) -> Optional[str]:
        ...
        unsupported = [
            'Table.Buffer', 'Table.Cache', 'List.Generate', 'Table.FromList',
            ...
            for col in sql_parts['select']:
                if col not in sql_parts.get('removed_columns', set()):
                    if col in sql_parts.get('column_mapping', {}):
                        select_items.append(f"{col} AS {sql_parts['column_mapping'][col]}")
                    else:
...
class DAXToSQLTranslator:
    ...
    def translate_column(self, column_def: Dict[str, Any], table_name: str) -> TranslationResult:
        ...
        try:
            ...
            warnings = [f"Remaining DAX: {', '.join(remaining_dax)}"] if remaining_dax else []
    ...
    def _translate_date_functions(self, dax: str) -> str:
        ...
        replacements = [
            ...
            (r'DATEDIFF\s*\(([^,]+),\s*([^,]+),\s*MONTH\)', r"DATEDIFF('month', \1, \2)"),
    ...
    def _check_remaining_dax(self, sql: str) -> List[str]:
        ...
        dax_functions = [
            ...
            'RELATED', 'RELATEDTABLE', 'SUMMARIZE', 'ADDCOLUMNS', 'SELECTCOLUMNS',
            'EARLIER', 'EARLIEST', 'VALUES', 'DISTINCT', 'COUNTROWS',
            'BLANK', 'ISBLANK', 'HASONEVALUE', 'SELECTEDVALUE'
        ]
    ...
    def _detect_table_pattern(self, dax: str, table_name: str) -> Optional[TranslationResult]:
    ...
    def _create_parameter_table_sql(self, dax: str, table_name: str) -> TranslationResult:
    ...
    def _create_summarize_table_sql(self, dax: str, table_name: str) -> TranslationResult:
            ...
            if calc_table_expr:
                ...
                if m_expr:
                    ...
                    for col in calculated_columns:
                        col_result = self.dax_translator.translate_column(col, table_name)
                        if col_result.success:
                            column_sql.append(col_result.sql)
                        else:
                            result['warnings'].append(f"Column {col.get('name')}: {col_result.warnings}")
